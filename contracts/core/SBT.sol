 ...
 * @author Faisal Naveed <faisal@genesislab.com>
 *
 */
...
contract SBT is Context, ERC165, IERC1155, IERC1155MetadataURI, IERC5516 {
    ...
    /**
     *
     * If action == true: mint pending token under `id`.
     *
     */
    ...
    /**
     * @dev See {eip-5516-mint}
     *
     * For each `id` - `action` pair:
     *
     * If action == true: Mints pending token under `id`.
     *
     */
    function claimAndMintBatch(
        ...
        _mintBatch(account, ids);
    }

    /**
     * @dev Mints or Reject pending token under `_id` from address `_account`.
     *
     * Requirements:
     *
     * - `account` cannot be the zero address.
    ...
    function _mint(
        ...
        _beforeTokenMint(operator, account, ids);

        _balances[account][id] = true;
        _pendings[account][id] = false;

        delete _pendings[account][id];

        emit TokenMinted(operator, account, ids);

        _afterTokenMint(operator, account, ids);
    }

    /**
     * @dev Mints or Reject _pendings `_id` from address `_account`.
     *
     * For each `id`-`action` pair:
     *
     * Requirements:
